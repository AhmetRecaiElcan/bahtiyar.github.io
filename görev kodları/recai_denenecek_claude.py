...
avoidance_stage = 0  # 0: normal, 1: durdur, 2: yan hareket, 3: düz git, 4: geri dön
obstacle_stop_time = 0  # Durma zamanı
...
def obstacle_avoidance_maneuver(obstacle_position):
    """Engel atlama manevrası - önce dur, sonra atla"""
    global obstacle_avoidance_active, avoidance_start_time, avoidance_stage, obstacle_stop_time
    ...
    if not obstacle_avoidance_active:
        ...
        obstacle_stop_time = current_time
        avoidance_stage = 1  # Önce durdur
        print(f"🚁 ENGEL ATLAMA BAŞLADI - Engel konumu: {obstacle_position}")
        print("⏸️ ÖNCE DURUYOR...")
    
    elapsed = current_time - avoidance_start_time
    
    if avoidance_stage == 1:  # Önce dur (1.5 saniye)
        send_rc(PWM_STOP, PWM_STOP)  # Tam dur
        print("⏸️ GEMİ DURUYOR - ENGEL ANALİZ EDİLİYOR")
        
        if elapsed > 1.5:  # 1.5 saniye dur
            avoidance_stage = 2
            avoidance_start_time = current_time
            print(f"🚀 ENGEL ATLAMA MANEVRASı BAŞLIYOR - {obstacle_position.upper()} TARAFI")
            
    elif avoidance_stage == 2:  # Yan hareket (2.5 saniye)
        if obstacle_position == "left":
            # Sol engel -> sağa git
            send_rc(PWM_FAST, PWM_STOP + 220)  # sağa dön (daha keskin)
            print("↗️ SOL ENGEL - SAĞA KAÇIYOR")
        elif obstacle_position == "right":
            # Sağ engel -> sola git  
            send_rc(PWM_FAST, PWM_STOP - 220)  # sola dön (daha keskin)
            print("↖️ SAĞ ENGEL - SOLA KAÇIYOR")
        else:  # center
            # Orta engel -> rastgele tarafa kaç (sağa tercih)
            direction = 1 if (current_time % 4) > 2 else -1
            send_rc(PWM_FAST, PWM_STOP + (220 * direction))
            print(f"{'↗️ ORTA ENGEL - SAĞA' if direction > 0 else '↖️ ORTA ENGEL - SOLA'} KAÇIYOR")
        
        if elapsed > 2.5:  # 2.5 saniye yan hareket (artırıldı)
            avoidance_stage = 3
            avoidance_start_time = current_time
            
    elif avoidance_stage == 3:  # Düz git (2 saniye)
        send_rc(PWM_FAST, PWM_STOP)
        print("➡️ ENGEL ATLAMA - ENGELDEN UZAKLAŞIYOR")
        
        if elapsed > 2.0:  # 2 saniye düz git (artırıldı)
            avoidance_stage = 4
            avoidance_start_time = current_time
            
    elif avoidance_stage == 4:  # Geri dön (2 saniye)
        if obstacle_position == "left":
            # Sola geri dön
            send_rc(PWM_FAST, PWM_STOP - 200)  # yumuşak geri dönüş
        ...
        if elapsed > 2.0:  # 2 saniye geri dön (artırıldı)
            # Manevra tamamlandı
            obstacle_avoidance_active = False
            avoidance_stage = 0
            print("✅ ENGEL ATLAMA TAMAMLANDI - GPS NAVİGASYONA GERİ DÖNÜYOR")
...
try:
    while True:
        ...
        if obstacle_avoidance_active:
            stage_names = ["", "DURUYOR", "Yan Hareket", "Düz Git", "Geri Dön"]
        ...
        if key == ord('q'):
            ...
            current_mode = "MANUAL"
            obstacle_detected = True
            obstacle_avoidance_active = False  # Reset önce
            avoidance_stage = 0
