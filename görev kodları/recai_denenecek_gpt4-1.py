...
"""
#Dostum buradaki engel atlama 
# kodunu sarı engeli görünce dursun gemi ardından atlama manevrası 
# yapsın bir süre atladığı yolda devam edip yine gpse gitsin
...
obstacle_waiting_to_jump = False  # Sarı engel görüldü, durdu, atlama bekliyor
obstacle_wait_start_time = 0
obstacle_post_jump_forward = False  # Atlama sonrası düz gitme
obstacle_post_jump_start_time = 0
    ...
    while True:
        ...
        if yellow_ratio > 0.15:
            if not obstacle_detected:
                ...
                # Önce dur, sonra atlama için bekle
                stop_all()
                obstacle_waiting_to_jump = True
                obstacle_wait_start_time = time.time()
        else:
            if obstacle_detected:
                print("✅ ENGEL TEMİZLENDİ")
            obstacle_detected = False
            obstacle_waiting_to_jump = False
        ...
        if current_mode == "AUTO_GPS" and current_lat is not None:
            if time.time() - last_nav_update > 0.05:  # daha hassas güncelleme
                # 1) Sarı engel görüldü, durdu, atlama için bekliyor mu?
                if obstacle_waiting_to_jump:
                    if time.time() - obstacle_wait_start_time > 1.0:  # 1 saniye bekle
                        print("⏳ ENGEL ATLAMA BAŞLIYOR!")
                        obstacle_avoidance_maneuver(obstacle_position)
                        obstacle_waiting_to_jump = False
                        last_nav_update = time.time()
                        continue
                    else:
                        # Hala bekleme süresi dolmadı, durmaya devam
                        stop_all()
                        last_nav_update = time.time()
                        continue
                # 2) Engel atlama aktifse sadece manevrayı çalıştır
                if obstacle_avoidance_active:
                    ...
                    if maneuver_active:
                        ...
                        # Manevra bitti, düz devam etme aşamasına geç
                        obstacle_post_jump_forward = True
                        obstacle_post_jump_start_time = time.time()
                        print("➡️ ATLAMA SONRASI DÜZ DEVAM EDİYOR")
                        # Düz gitmeye başla
                        send_rc(PWM_FAST, PWM_STOP)
                        update_simulated_position(PWM_FAST, PWM_STOP)
                        last_nav_update = time.time()
                        continue
                # 3) Atlama sonrası düz gitme aşaması
                if obstacle_post_jump_forward:
                    if time.time() - obstacle_post_jump_start_time < 2.0:  # 2 saniye düz git
                        send_rc(PWM_FAST, PWM_STOP)
                        update_simulated_position(PWM_FAST, PWM_STOP)
                        last_nav_update = time.time()
                        continue
                    else:
                        obstacle_post_jump_forward = False
                        print("🔄 GPS NAVİGASYONA DÖNÜYOR")
                # 4) Normal GPS navigasyon (engel yoksa veya manevra bitmişse)
                if not obstacle_detected and not obstacle_avoidance_active and not obstacle_post_jump_forward:
